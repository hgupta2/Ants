...
public class Ants {
    ...
    private HashMap<Tile, Boolean> explored = new HashMap<Tile, Boolean>();
    ...
    public Ants(int loadTime, int turnTime, int rows, int cols, int turns, int viewRadius2,
        ...
        for (int row = 0; row < rows; row++) {
			for (int col = 0; col < cols; col++) {
				...
				Tile n = mapTiles[(row+rows-1)%rows][col];
				Tile e = mapTiles[row][(col+1)%cols];
				Tile s = mapTiles[(row+1)%rows][col];
				Tile w = mapTiles[row][(col+cols-1)%cols];
				if (((col + row) & 1) == 1) {
					tile.setNeighbors(0, n);
					tile.setNeighbors(1, s);
					tile.setNeighbors(2, e);
					tile.setNeighbors(3, w);
				} else {
					tile.setNeighbors(0, e);
					tile.setNeighbors(1, w);
					tile.setNeighbors(2, n);
					tile.setNeighbors(3, s);
				}
			}
		}
        ...
        for (int row = 0; row < rows; row++) {
			for (int col = 0; col < cols; col++) {
				explored.put(mapTiles[row][col], false);
			}
		}
    ...
    /**
     * Returns Explored Tiles.
     * 
     * @return Tile[][]
     */
    public HashMap<Tile, Boolean> getExploredTiles(){
    	return explored;
    }
     ...
     * 
     */
    public void setExploredTiles(HashMap<Tile, Boolean> exp){
    	explored = exp;
    }
    /**
    ...
    public void update(Ilk ilk, Tile tile) {
        ...
        switch (ilk) {
            ...
            break;
            case WATER:
            	for(Tile neighbor : mapTiles[tile.getRow()][tile.getCol()].getNeighbors()){
                		neighbor.removeNeighbor(mapTiles[tile.getRow()][tile.getCol()]);
                }
            break;
        }
    }
    ...
    /** Remove the tile from the neighbors which contains my ants.
     *  
     * 
     */
    public void updateMapTiles(){
    	for(int i=0; i<rows; i++){
    		for(int j=0; j<cols; j++){
    			...
    			if(myAnts.contains(tile) || myHills.contains(tile) || enemyAnts.contains(tile) || enemyHills.contains(tile)){
    				for(Tile neighbor : tile.getNeighbors()){
    					neighbor.removeNeighbor(tile);
    				}
    			}
    		}
    	}
